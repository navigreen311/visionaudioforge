...
export default function RTSPConnector({ onConnect, onDisconnect }: RTSPConnectorProps) {
  ...
  const handleTest = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await api.get<RTSPTestResponse>("/api/capture/rtsp/test", {
        params: { url: fullUrl },
      });
      setTestResult(data);
      if (data.status === "ok") {
        ...
        addLog(`Test failed: ${data.error ?? "Unknown error"}`, "error");
      }
    } catch (err: unknown) {
      const message =
  ...
  const handleConnect = useCallback(() => {
    ...
    // Simulate a small delay so the UI shows a loading state before the
    // parent takes over the stream handling.
    setTimeout(() => {
      setConnected(true);
      setConnecting(false);
  ...
  const canConnect = url.trim().length > 0 && !connected && !connecting;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="border border-gray-200 rounded-lg overflow-hidden">
        ...
        {authOpen && (
          <div className="px-4 py-3 space-y-3 border-t border-gray-200">
            <div>
              <label htmlFor="rtsp-user" className="block text-sm font-medium text-gray-600 mb-1">
                Username
              </label>
              <input
                id="rtsp-user"
            ...
            <div>
              <label htmlFor="rtsp-pass" className="block text-sm font-medium text-gray-600 mb-1">
                Password
              </label>
              <input
                id="rtsp-pass"
          ...
          {testResult.status === "ok" ? (
            ...
            <>
              ...
              <span className="text-red-600">
                &mdash; {testResult.error ?? "Unknown error"}
              </span>
            </>
          )}
        </div>
      ...
      <div className="flex items-center gap-3">
        <button
          onClick={handleTest}
          disabled={!url.trim() || testing || connected}
