...
class VisionClient:
    """Wraps the /api/v1/vision endpoints."""
    ...
    async def analyze(self, image_path: str, operations: list[str]) -> AnalysisResult:
        ...
        resp = await self._client._request("POST", "/api/v1/vision/analyze", files=files, data=data)
    ...
    async def detect(self, image_path: str, confidence: float = 0.5) -> list[Detection]:
        ...
        resp = await self._client._request("POST", "/api/v1/vision/detect", files=files, data=data)
    ...
    async def optical_flow(
        ...
        resp = await self._client._request("POST", "/api/v1/vision/optical-flow", files=files, data=data)
        return FlowResult.model_validate(resp)

    async def ocr(self, image_path: str) -> OCRResult:
        """Extract text from an image."""
        files = {"file": ("image", Path(image_path).read_bytes())}
        resp = await self._client._request("POST", "/api/v1/vision/ocr", files=files)
    ...
    async def segment(self, image_path: str) -> SegmentationResult:
        ...
        files = {"file": ("image", Path(image_path).read_bytes())}
        resp = await self._client._request("POST", "/api/v1/vision/segment", files=files)
    ...
    async def track(self, frame_paths: list[str]) -> TrackingResult:
        ...
        resp = await self._client._request("POST", "/api/v1/vision/track", files=files)
